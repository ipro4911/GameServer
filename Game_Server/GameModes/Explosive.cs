...
//        {
//            if (room.mode == 0 && room.channel == 1 && !room.isSpawned)
//            {
    ...
    class Explosive
    {
        ...
        public void sendNewRound(int WinningTeam)
        {
            if (room.waitExplosiveTime >= 5 && !room.EndGamefreeze)
            {
                ...
                room.updateTime();
                room.timespent = 0;
                room.timeleft = room.mode != (int)RoomMode.Annihilation ? 180000 : 90000;
                room.bombDefused = false;
                room.bombPlanted = false;
                room.sleep = false;
                room.isNewRound = false;
